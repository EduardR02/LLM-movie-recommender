"""Grok analysis pipeline using xAI's AsyncClient."""
...
def _is_rate_limited_error(exc: Exception) -> bool:
    """Return True if the exception represents a Grok rate limit."""
    if grpc is not None:
        ...
        if status_enum and rpc_error_cls and isinstance(exc, rpc_error_cls):
            try:
                ...
                pass
    message = str(exc)
    return "RESOURCE_EXHAUSTED" in message or "Too many requests" in message


class AnalysisRunner:
    """Generate rich analyses for plots using Grok."""
    ...
    def run(self, *, limit: int | None = None, force: bool = False) -> None:
        ...
        session_id = f"grok-{uuid.uuid4().hex[:10]}"
        component_name = f"grok_analysis[{self.profile}]"
        self.manifest.register_session(session_id, component=component_name)

        LOGGER.info("Starting Grok analysis for %d titles (session=%s)", len(candidates), session_id)
        totals = UsageTotals()
        try:
            ...
            total_cost = self._estimate_cost(totals.prompt_tokens, totals.completion_tokens)
            ...
            LOGGER.info(
                "Grok analysis session complete: prompt=%d (cached=%d) completion=%d (reasoning=%d)",
        ...
        *,
        ...
        for attempt_index in range(max_attempts):
            ...
            try:
                ...
                LOGGER.error(
                    "Grok request failed for %s (attempt %d/%d): %s",
                    title.primary_title,
                    attempt_index + 1,
                    max_attempts,
                    exc,
            ...
            self.manifest.update_analysis_status(
                ...
                cost_estimate=self._estimate_cost(result.prompt_tokens, result.completion_tokens),
    ...
    def _estimate_cost(self, prompt_tokens: int, completion_tokens: int) -> float:
        prompt_cost = (prompt_tokens / 1_000_000) * self.config.input_cost_per_mtoken
